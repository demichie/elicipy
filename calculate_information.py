def calculate_information(Cal_var, TQs, realization, k, background_measure):
        ...
        for e in np.arange(E):
            ...
            if background_measure[i]=='uni':
                ...
                if l[i]<0 or h[i]<0:
                    ...
                    raise ValueError('Log-Uniform background measure cannot be used for item %d because an expert provided negative values',i) 
    ...
    N_items_tot = Cal_var.shape[2] + TQs.shape[2]
    sumb = np.zeros((N_items_tot,E))
    

    for i in np.arange(Cal_var.shape[2],N_items_tot):
        if not realization[i]: # information of Target Questions
        
            # print(i,i-Cal_var.shape[2])
            l[i] = np.amin(TQs[:,:,i-Cal_var.shape[2]])
            h[i] = np.amax(TQs[:,:,i-Cal_var.shape[2]])
            ...
            for e in np.arange(E):
                if background_measure[i]=='uni':
                    ...
                    tmp = np.insert(TQs[e,:,i-Cal_var.shape[2]],0,x_o[i])
                    tmp2 = np.append(tmp,x_n[i])
                    x[e,:] = tmp2
                    ...
                    tmp = np.insert(np.log(TQs[e,:,i-Cal_var.shape[2]]),0,x_o[i])
                    tmp2 = np.append(tmp,x_n[i])
                    x[e,:] = tmp2
                    
                    
                for j in np.arange(N_P):
                    sumb[i-Cal_var.shape[2],e] = sumb[i-Cal_var.shape[2],e] + p[j] * np.log( p[j] / (x[e,j+1] - x[e,j]) )
            
                info_per_variable[i,e] = np.log(x_n[i] - x_o[i]) + sumb[i-Cal_var.shape[2],e]
