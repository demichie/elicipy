...
def github_file_to_bytes(Repository,repo, filename, branch="main"):
    content_encoded = repo.get_contents(urllib.parse.quote(filename), ref=branch).content
    ...
    for content in contents:
        ...
        if content.type == "dir":
            ...
            try:
                path = content.path
                if (path[-3:]=='csv'):
                    content_encoded = repository.get_contents(urllib.parse.quote(path), ref=branch)
